    ...
    async def generate_response(
        ...
        if provider:
            if provider == "openai" and settings.llm.openai_api_key:
                return await self.call_openai_api(prompt, model, system_prompt, temperature, max_tokens)
            elif provider == "anthropic" and settings.llm.anthropic_api_key:
                return await self.call_anthropic_api(prompt, model, system_prompt, temperature, max_tokens)
            elif provider == "ollama" and settings.llm.ollama_base_url:
                return await self.call_ollama_api(prompt, model, system_prompt, temperature, max_tokens)
            else:
